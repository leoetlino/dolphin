...
struct BigEndianValue
{
  ...
  operator value_type() const { return Common::FromBigEndian(raw); }
  void operator=(value_type v) { raw = Common::FromBigEndian(v); }
private:
  value_type raw;
};
...
struct FstEntry
{
  ...
  Mode GetOwnerMode() const;
  Mode GetGroupMode() const;
  Mode GetOtherMode() const;
  void SetAccessMode(Mode owner, Mode group, Mode other);
